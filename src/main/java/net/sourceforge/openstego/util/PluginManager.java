...
public class PluginManager {
  ...
  public static void loadPlugins() throws OpenStegoException {
    ...
    try {
      InputStream stream = PluginManager.class.getClassLoader().getResourceAsStream("src/main/resources/OpenStegoPlugins.internal");
      System.out.println(stream != null);

      // Load internal plugins
      is = PluginManager.class.getClassLoader().getResourceAsStream("OpenStegoPlugins.internal");
      pluginList = StringUtil.getStringLines(new String(CommonUtil.getStreamBytes(is)));

      // Load external plugins if available
      is = PluginManager.class.getClassLoader().getResourceAsStream("OpenStegoPlugins.external");
      if (is != null) {
        pluginList.addAll(StringUtil.getStringLines(new String(CommonUtil.getStreamBytes(is))));
      }

      for (int i = 0; i < pluginList.size(); i++) {
        String classname = pluginList.get(i);
